...
pub fn estimate_pi(total_points: i32, total_threads: i32) {
    ...
    compute_on_multiple_threads(total_points, total_threads, tx);
...
fn compute_on_multiple_threads(total_count: i32, num_of_threads: i32, tx: Sender<i32>) {
    for _i in 0..num_of_threads {
        ...
        thread::spawn(move || {
            ...
            new_tx.send(in_circle_count).unwrap();
        });
    }
}
